...
export async function validateCatalogWithSparql(
  reporter: ValidationReporter, content: RDF.Quad[]
): Promise<void> {
...
export async function validateDatasetWithSparql(
  reporter: ValidationReporter, content: RDF.Quad[], dataset:string
): Promise<void> {
  const engine = await prepareEngine(content);
...
async function validateDatasets(
  reporter: ValidationReporter, engine: Engine, dataset:string
): Promise<undefined> {
